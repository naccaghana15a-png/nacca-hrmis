...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="flex-1">
                  <p className="text-sm m-0">
                    ...
                    <span className={`ml-2 text-xs font-semibold ${a.isApproval ? 'text-green-600' : 'text-blue-600'}`}>
                      <i className={`fas ${a.isApproval ? 'fa-check-circle' : 'fa-paper-plane'} mr-1`}></i>
                      {a.isApproval ? 'Approval' : 'Submission'}
                    </span>
                    <br />
        ...
        <div className="content-card">
          ...
          <div className="p-4 space-y-2">
            <button className="quick-action" onClick={() => window.location.href = '/admin/leave'}>
              <i className="fas fa-calendar-plus w-5"></i> Apply Leave
            </button>
            <button className="quick-action">
              <i className="fas fa-clock w-5"></i> My Attendance
            </button>
            {user?.role === 'SUPER_ADMIN' && (
              <>
                <button className="quick-action" onClick={() => window.location.href = '/admin/employees'}>
                  <i className="fas fa-user-cog w-5"></i> Manage Employees
                </button>
                <button className="quick-action">
                  <i className="fas fa-database w-5"></i> System Backup
                </button>
              </>
            )}
            {user?.role === 'DIRECTOR' && (
              <>
                <button className="quick-action">
                  <i className="fas fa-users w-5"></i> Team Dashboard
                </button>
                <button className="quick-action">
                  <i className="fas fa-file-alt w-5"></i> Department Reports
                </button>
              </>
            )}
          </div>
      ...
      {/* Pending Approvals (Super Admin & DG only) */}
      {(user?.role === 'SUPER_ADMIN' || user?.role === 'DIRECTOR_GENERAL') && (
        <div className="content-card">
          <div className="px-5 py-3 border-b border-[#e2e8f0] flex items-center justify-between">
            <h5 className="font-semibold flex items-center gap-2">
